...
PlayerShip::PlayerShip(sf::Image &spritesheet, int startx): spritesheet(spritesheet), x(startx)
{
    ...
    this->alive_sprite.setPosition(this->x, 593); // 593 is the y pos of the ship, which never changes.
...
{
    this->alive_sprite.move(this->move_speed * dir, 0);
    this->x += (this->move_speed * dir); // To keep track of where the ship is on screen
}
